...
The standard `denoise()` workflow also fits a two-component native-plus-ambient mixture. It retains the fixed estimate when the two estimators agree and selects the mixture estimate when their disagreement is large. Agreement is a robustness switch, not proof that the two estimators measure the same physical contamination rate. Executed $\rho_c$ is then multiplied by a sample-level scale $s(q)$, $q=\mathrm{median}(\hat\rho)\,\bar n/\lambda_e$, where $\lambda_e$ is the mean empty-droplet UMI. $s(q)$ shrinks when $q$ is low and expands when $q$ is high (floor $0.50$, cap $1.25$). When $s(q)<1$, shrink is blended toward $1$ as median selected $\hat\rho$ approaches $0$, so a clean library is not scaled like a high-contamination run with fat empties. Cell ranking is unchanged. For a sample with only one usable cell type, it uses the untyped $\chi$ quantile floor instead of a two-component fit, then the same scale. `estimate_dose()` exposes the unscaled fixed estimator used within this workflow; calling it directly is not an alternative product path. Missing $\lambda_e$ (no empty droplets recorded by `estimate_chi`) raises.
...
That identity is the rank-1 channel only. Default subtraction is dual-channel and type-constrained: a type budget $D_t=\sum_{c\in t}d_c$, unsaturated unprotected rank-1 spent inside cells in proportion to $d_c$, saturated gene columns on a type-level allocator, protected genes allocated by library size $n_c$, and soupOnly extra-clear of unexpressed unowned genes that may exceed $d_c$. High-$\chi$ U extra-clear uses an 80\% $\chi$-mass prefix of U genes. Native confidence scales the $\chi$-direction rank-1 take (10\% of the unprotected slice at confidence 1); that is not a universal cap on native UMI loss.

The default subtraction algorithm aggregates by type. Rank-1 take is `dose·χ·(1 − 0.9·native_confidence)` with `dose` equal to $\sum_{c\in t}d_c$, never exceeding that χ-direction budget on those genes. Unused budget after clipping on protected genes is reallocated along χ to genes that are neither native-protected nor soupOnly, and not to genes already above the $\rho=1$ ceiling (`mean/(n̄χ)>1`). Unowned rank-1 and soupOnly takes are then blended 10% toward `take × max(Pearson(y/n, ρ), 0)` so a saturated gene×type budget cannot fully clear cells whose counts do not track per-cell dose. Rank-1 genes that already meet the type's observed capacity *before* that blend stay on the type-level integer allocator; unsaturated rank-1 take is spent inside each cell, so leftover fractional χ-budget is not taken from other cells of the same type. soupOnly extra-clear stays type-pooled. Per-cell $d_c$ is that budget's cell-level counterpart, not a per-gene weight. Owned genes keep library-size weights and are not enrichment-scaled. Unexpressed unowned genes are extra-cleared (`soupOnly`) only when the type's dose-weighted $\rho$ is at least 0.01 and `mean/(n̄χ)` is well below the ambient ceiling (below 0.4); that extra-clear is not scaled back to $d_c$. SoupOnly is also revoked when this type is the exclusive $r_t$ argmax (fold over the runner-up), so a high-$\chi$ identity gene is not extra-cleared in its leading type even if $r_t<0.4$. Owned genes (ownership can be shared by more than one meta-group when each clears the ownership fold threshold over the next-lower tier, not only a single global winner), mitochondrial genes, genes that look native in every type, and genes sitting on the $\rho=1$ ambient ceiling (`mean/(n̄χ)>0.85`) are excluded from extra-clear. After the high-$\chi$ unique-argmax prefix, a type also loses ownership of a gene when its $r_t\le 1$ **and** some type in the sample has $r_t>1$.
