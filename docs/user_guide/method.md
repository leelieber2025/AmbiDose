...
That identity is the rank-1 channel only. Default subtraction is dual-channel and type-constrained: a type budget $D_t=\sum_{c\in t}d_c$, unsaturated unprotected rank-1 spent inside cells in proportion to $d_c$, saturated gene columns on a type-level allocator, and soupOnly extra-clear of unexpressed unowned genes that may exceed $d_c$. Protected-gene takes use a within-type reliability-weighted blend of dose rank and an anchor rate measured from soupOnly evidence genes; if the anchor has no noise-resolved signal, the allocation falls back to dose rank. High-$\chi$ U extra-clear uses an 80\% $\chi$-mass prefix of U genes. Native confidence scales the initial $\chi$-direction rank-1 take (10\% of the unprotected slice at confidence 1); that is not a universal cap on native UMI loss.

The default subtraction algorithm aggregates by type. Initial rank-1 take is `dose·χ·(1 − 0.9·native_confidence)` with `dose` equal to $\sum_{c\in t}d_c$. Unused budget after clipping is evaluated jointly across protected and unprotected non-soupOnly genes in the same library and type; there is no fixed pool split and no cross-library calibration. Targets above the physical $\rho=1$ ceiling (`mean/(n̄χ)>1`) are ineligible. A nonnegative within-type slope of normalized gene abundance against cell-specific $\rho$, relative to $\chi$, supplies ambient-response evidence. For unprotected genes it limits both priority and removable capacity; for protected genes it is multiplied by one minus native confidence. Unsupported residual budget remains unspent instead of being normalized into forced deletion. soupOnly eligibility and its high-/low-$\chi$ remaining-dose rules are unchanged. All paths remain capped by observed counts and produce nonnegative integer output.
