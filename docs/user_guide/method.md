# Method

## Scope

AmbiDose is a count-removal operator for droplet scRNA-seq. Empty droplets identify ambient composition; broad cell populations identify genes unlikely to contain native expression; cell depth converts a contamination fraction into an absolute UMI dose.

Cell calling is an input-preparation step rather than the inferential target. AmbiDose is not a batch integration model or a posterior generative denoiser.

## Ambient profile

For empty droplet $e$ in sample $s$,

$$y_e \sim \operatorname{Poisson}(n_e\chi_s), \qquad \sum_g \chi_{s,g}=1.$$

The implementation estimates $\chi_s$ by summing counts over that sample's empty droplets and normalizing the gene totals to a simplex. Profiles are never shared across samples.

## Per-cell dose

For cell $c$ in sample $s$,

$$y_c \approx \text{native}_c + d_c\chi_s, \qquad d_c=\rho_c n_c.$$

Here $n_c$ is the cell's observed library size and $\rho_c\in[0,1]$ an operational scale. $d_c$ is the $\chi$-direction rank-1 budget: the mass allocated along $\chi_s$. It is not a calibrated contamination rate and not a cap on the cell's total UMI removal.

For a broad type $t$, AmbiDose compares the observed type mean with the ambient-only ceiling $\bar n_t\chi_s$, including a Poisson noise margin. Genes consistent with ambient-only expression form the evidence pool. Genes that look native in every type — mean above that ceiling everywhere, or a MALAT1-like collision ($r_t>0.85$ and a large library fraction in every type) — are dropped. True soup sits at or below the ceiling in every type and is kept; a cross-type fold filter is not used, because rank-1 ambient is uniform by construction. The fixed estimator uses a Poisson maximum-likelihood estimate when sufficient type-specific evidence is available and otherwise uses a conservative quantile estimate over high-$\chi$ genes. Raw log-$\rho$ values are shrunk toward the within-sample median according to the number of observed evidence genes.

The standard `denoise()` workflow also fits a two-component native-plus-ambient mixture. It retains the fixed estimate when the two estimators agree and selects the mixture estimate when their disagreement is large; the disagreement threshold is the Poisson sampling error of the two estimates, not a fitted fold/gap constant. Agreement is a robustness switch, not proof that the two estimators measure the same physical contamination rate. Executed $\rho_c$ is the selected estimate itself: a sample-level rescaling $s(q)$ fitted on prior evaluation panels was removed because it did not generalize. $q=\mathrm{median}(\hat\rho)\,\bar n/\lambda_e$, where $\lambda_e$ is the mean empty-droplet UMI, is still recorded per sample as a diagnostic. Cell ranking is unchanged. For a sample with only one usable cell type, it uses the untyped $\chi$ quantile floor instead of a two-component fit. `estimate_dose()` exposes the fixed estimator used within this workflow; calling it directly is not an alternative product path. Missing $\lambda_e$ (no empty droplets recorded by `estimate_chi`) raises.

## Type-aware subtraction

The elementary rank-one removal is

$$\tilde y_{c,g}=\max(y_{c,g}-d_c\chi_{s,g},0).$$

That identity is the rank-1 channel only. Default subtraction is dual-channel and type-constrained: a type budget $D_t=\sum_{c\in t}d_c$, unsaturated unprotected rank-1 spent inside cells in proportion to $d_c$, saturated gene columns on a type-level allocator, and soupOnly extra-clear of unexpressed unowned genes that may exceed $d_c$. Protected-gene takes use a within-type reliability-weighted blend of dose rank and an anchor rate measured from soupOnly evidence genes; if the anchor has no noise-resolved signal, the allocation falls back to dose rank. SoupOnly extra-clear covers the full $\chi$ mass of unexpressed unowned (U) genes (no high-/low-$\chi$ split), capped by default at the remaining $d_c$ after rank-1 with no soft allowance beyond it. A gene is eligible only when its type mean is compatible with that type's own $\rho$ under a Poisson sampling margin, not a fixed cross-type ratio cut. Extra-clear and any leftover-budget reallocation are skipped outright for a type-sample group whose total unexpressed-unowned UMI mass, or estimated per-cell soup ($\rho_c n_c$), is already consistent with the empty-droplet background under a Poisson-style margin; `uns['ambidose']['n_empty_consistent_skip_cells']` reports the number of cells this affected. The same skip also triggers when only *another* type's owned U genes are elevated in this group's evidence and that mass alone matches the empty-droplet background: it is read as $\chi$ overlapping native composition, not genuine cross-type soup. When a group's estimated per-cell soup sits above the empty-droplet ceiling but no more than 3$\times$ it, native confidence no longer scales the rank-1 take at all (the full $\chi$-direction quantity is taken, as a single-fraction estimator would); above that band, native-confidence scaling applies as usual. Within that same band, $\chi$ itself is renormalized toward unexpressed and low-confidence genes before the take is computed, so the budget lands on likely-soup genes first rather than in proportion to raw $\chi$, where high-confidence native genes would otherwise absorb it. Native confidence scales the initial $\chi$-direction rank-1 take (10\% of the unprotected slice at confidence 1); that is not a universal cap on native UMI loss.

The default subtraction algorithm aggregates by type. Initial rank-1 take is `dose·χ·(1 − 0.9·native_confidence)` with `dose` equal to $\sum_{c\in t}d_c$. Unused budget after clipping is evaluated jointly across protected and unprotected non-soupOnly genes in the same library and type; there is no fixed pool split and no cross-library calibration. Targets above the physical $\rho=1$ ceiling (`mean/(n̄χ)>1`) are ineligible. A nonnegative within-type slope of normalized gene abundance against cell-specific $\rho$, relative to $\chi$, supplies ambient-response evidence. For unprotected genes it limits both priority and removable capacity; for protected genes it is multiplied by one minus native confidence. Unsupported residual budget remains unspent instead of being normalized into forced deletion. SoupOnly eligibility and its remaining-dose cap are as described above. All paths remain capped by observed counts and produce nonnegative integer output.

The default cross-type protection is applied only during subtraction. A candidate gene must first show cross-cell expression structure associated with latent programs derived from confident-owner genes; it must then exceed the cross-type anchor ratio. This two-stage rule protects broadly distributed native signal without allowing a uniformly distributed contaminant to serve as its own evidence. Dose estimation is unchanged. Set `cross_type_anchor=False` only to reproduce the reference ablation used in method comparisons.

The final matrix is rounded to nonnegative `int32` counts. Removal only acts on existing sparse entries, so it cannot introduce expression at a previously zero gene and cannot write a count above raw.

## Why broad types matter

A high-$\chi$ gene can be native in one lineage and ambient in another. A single global gene list cannot distinguish those cases. Broad types supply the minimum biological structure needed for that distinction; fine annotation is not the objective and is not produced by AmbiDose. Automatic typing is label-free Leiden within each library. It does not name cell types and does not call external annotators. Groups with fewer than `MIN_TYPE_CELLS=10` cells skip type-level extra-clear and use the conservative rank-1 protected path, so results can change discontinuously at that threshold; the QC label `type_structure_risk` marks these small groups. A higher Leiden resolution is not a more accurate correction: it makes gene ownership harder to establish and can increase over-removal of housekeeping-shaped genes. Pass `type_key` only for independently established broad labels.

## Known limitation

Genes with similar expression across all broad types, including many ribosomal, mitochondrial, and housekeeping genes, may lie near the $\rho=1$ ambient ceiling even when contamination is low. Their inclusion in the MLE evidence pool can produce an upward bias in $\rho$ at low true contamination. At high true contamination the same estimator can *under*-estimate $\rho$ and subtract far less than the true ambient mass; on toys the mapping from true $\rho$ to $\hat\rho$ is not monotone. An iterative estimator corrected the low-$\rho$ bias in a synthetic calibration experiment but reduced performance on real benchmark datasets and is therefore not used by `denoise()`. Later diagnostics (uniform-gene $r$ bimodality, alternative evidence-pool ranking, two-component kick of high-$r$ genes) either failed to separate soup from housekeeping on tissue or under-estimated $\rho$ the same way the iterative estimator did. There is currently no promoted fix.

Treat `ambidose_rho` as an operational scale and `ambidose_d` as the $\chi$-direction rank-1 budget. Count monotonicity (`0 \le` corrected `\le` raw) is numerical conservatism, not a guarantee that native molecules were spared. `denoise()` writes `obs["ambidose_rho_trust"]` (`ok`, `low_evidence`, `ceiling_risk`, `type_structure_risk`, `under_execution`, `over_removal`) without changing the estimator. These are run-state labels, not a calibrated probability that the cell was correctly corrected. `under_execution` means less than half of $d_c$ was removed. `over_removal` means total removal exceeded $d_c$ by more than 5% (expected when soupOnly extra-clear is active) or exceeded half of the cell total. Inspect the QC report and validate retention of biologically important genes for each new tissue.

## Assumptions

- Input values are raw integer UMIs.
- Empty droplets and cells come from the same library.
- Each multi-sample library has enough retained empty droplets.
- Broad labels describe populations with reasonably comparable biology.
- Ambient composition is rank-one within a sample.
